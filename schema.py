...
class VystupSOhlasmi(MappingSchema):
  ...
  vramci_pomeru_s = SchemaNode(String(), title='V rámci pracoveného pomeru s', description=u'Uvádza sa v rámci pracovného pomeru s ktorou právnickou osobou, predmetné výstupy vznikli, boli publikované, teda či ide o tvorbu v rámci vysokej školy, ktorá podáva žiadosť.')
  ohlasy = SchemaNode(Sequence(), SchemaNode(String(), title='', name='ohlas', widget=deform.widget.TextAreaWidget(rows=2)), title=u'Ohlasy', validator=Length(max=10), widget=deform.widget.SequenceWidget(max_len=10))
...
class Charakteristika(MappingSchema):
  ...
  akt_pedag_cinnost = SchemaNode(Sequence(), PedagogickaCinnost(name='pedagogicka_cinnost', title=u'Pedagogická činnosť'), title=u'Aktuálna pedagogická činnosť', description=u'Uvádza sa zoznam aktuálne vyučovaných predmetov. Uvádzajú sa len predmety vyučované v rámci vysokej školy, ktorá predkladá žiadosť.')
  predch_pedag_cinnost = SchemaNode(Sequence(), PredchadzajucaPredagogickaCinnost(name='predchadzajuca_pedagogicka_cinnost', title=u'Pedagogická činnosť'), title=u'Predchádzajúca pedagogická činnosť', description=u'Uvádza sa zoznam najviac piatich vybraných, v minulosti vyučovaných predmetov. Ak bol predmet vyučovaný na inej vysokej škole, uvádza sa aj názov vysokej školy, na ktorej bol predmet vyučovaný', validator=Length(max=5), widget=deform.widget.SequenceWidget(max_len=5))
  tvoriva_cinnost = SchemaNode(Sequence(), TvorivaCinnost(name='tvoriva_cinnost', title=u'Tvorivá činnosť'), title=u'Aktuálna tvorivá činnosť', description=u'Uvádza sa zoznam aktuálnych výskumných projektov alebo umeleckých projektov, na ktorých riešení zamestnanec participuje. Ak ide o vedúceho projektu, uvádza sa táto skutočnosť osobitne. Je možné uviesť aj výskumné projekty pre hospodársku sféru.')
  ...
  najv_prace_celkom = SchemaNode(Sequence(), VystupBiblio(name='najv_prace_celkom', title=''), title=u'Najvýznamnejšie publikované vedecké práce, verejne realizované alebo prezentované umelecké diela a výkony.', description=u'Uvádza sa najviac päť výstupov.', validator=Length(max=5), widget=deform.widget.SequenceWidget(max_len=5))
  najv_prace_za_poslednych_6_rokov = SchemaNode(Sequence(), VystupBiblio(name='najv_prace_za_poslednych_6_rokov', title=''), title=u'Najvýznamnejšie publikované vedecké práce verejne realizované alebo prezentované umelecké diela alebo výkony za posledných šesť rokov.', description=u'Uvádza sa najviac päť výstupov.', validator=Length(max=5), widget=deform.widget.SequenceWidget(max_len=5))
  najv_projekty_za_poslednych_6_rokov = SchemaNode(Sequence(), VystupProjekt(name='najv_projekty_za_poslednych_6_rokov', title=''), title=u'Účasť na riešení (vedení) najvýznamnejších vedeckých projektov alebo umeleckých projektov za posledných šesť rokov.', description=u'Uvádzajú sa len projekty v pozícií zodpovedného riešiteľa a jeho zástupcu. Uvádza sa najviac päť projektov.', validator=Length(max=5), widget=deform.widget.SequenceWidget(max_len=5))
  vystupy_s_ohlasmi = SchemaNode(Sequence(), VystupSOhlasmi(name='vystup_s_ohlasmi', title=u'Výstup s ohlasmi'), title=u'Výstupy v oblasti poznania príslušného študijného odboru s najvýznamnejšími ohlasmi a prehľad ohlasov na tieto výstupy.', description=u'Uvádza sa najviac päť výstupov s najvýznamnejšími ohlasmi. Okrem bibliografických údajov o výstupe sa uvádzajú aj informácie o jednotlivých ohlasoch – vrátane databázy, v ktorej je ohlas evidovaný. Uvádza sa najviac desať ohlasov na jeden výstup, z ktorých najmenej jeden vznikol v predchádzajúcich šiestich rokoch.', validator=Length(max=5), widget=deform.widget.SequenceWidget(max_len=5))
  ...
  dopln_program = SchemaNode(String(), title=u'Charakteristika aktivít súvisiacich s príslušným študijným programom', widget=deform.widget.TextAreaWidget(rows=5), missing='', description=u'Uvádza sa len u garanta a spolugaranta študijného programu. Zabezpečované aktivity by mali preukázať, že garant má podstatný vplyv na uskutočňovanie študijného programu. Maximálne 3500 znakov.', validator=Length(max=3500))
  dopln_dalsie = SchemaNode(String(), title=u'Ďalšie aktivity', widget=deform.widget.TextAreaWidget(rows=5), missing='', description=u'Ak je to podstatné, uvádzajú sa iné aktivity súvisiace s vysokoškolským vzdelávaním alebo tvorivou činnosťou. Maximálne 3500 znakov.', validator=Length(max=3500))